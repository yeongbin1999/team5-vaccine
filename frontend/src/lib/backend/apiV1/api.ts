...
export interface PageResponseDtoOrderListDTO {
...
> extends HttpClient<SecurityDataType> {
  api = {
    ...
    /**
     * @description 장바구니 내용을 기반으로 주문을 생성합니다. (결제 전 단계) - 상품 재고를 자동으로 차감합니다. - 재고 부족시 InsufficientStockException이 발생합니다. - 주문 생성 후 '배송준비중' 상태로 설정됩니다.
     *
     * @tags Order
     * @name CreateOrder
     * @summary 주문 생성
    ...
    getAllOrders: (
      ...
      this.request<PageResponseDtoOrderListDTO, any>({
     ...
     *
     * @tags Admin Order
     * @name GetOrderDetail1
     * @summary 관리자 - 특정 주문 상세 조회
     * @request GET:/api/v1/admin/orders/{orderId}
     */
    getOrderDetail1: (orderId: number, params: RequestParams = {}) =>
      this.request<OrderDetailDTO, any>({
     ...
     * @summary 관리자 - 주문 통계 조회
     * @request GET:/api/v1/admin/orders/statistics
     */
    getOrderStatistics: (
      query: {
        /**
      ...
      this.request<OrderStatisticsResponseDto[], any>({
     ...
     *
     * @tags Admin Order
     * @name GetAllOrdersWithoutPaging
     * @summary 관리자 - 모든 주문 조회 (전체)
     * @request GET:/api/v1/admin/orders/all
     */
