      ...
      <div className="w-1/3 flex flex-col justify-center items-center bg-white">
        <div className="w-full max-w-md">
          <h1 className="text-2xl font-bold mb-8 text-gray-900 flex items-center gap-2">
            <Image src="/coffee.jpeg" alt="로고" width={36} height={36} className="inline-block" />
          ...
          {!isAuthenticated ? (
            ...
            <div className="flex flex-col gap-4">
              ...
              <button className="w-full bg-red-500 text-white py-2 rounded text-lg font-semibold transition hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400" onClick={logout}>로그아웃</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="w-2/3 h-full relative flex items-center justify-center">
        ...
        <div className="relative z-10 w-full flex justify-center items-center">
          <div className="bg-white bg-opacity-90 rounded-xl shadow-lg p-8 w-full max-w-4xl">
            {isAuthenticated && selectedMenu === 'product' && <ProductManagement />}
            {isAuthenticated && selectedMenu === 'order' && <div>주문관리 컴포넌트</div>}
            {isAuthenticated && selectedMenu === 'user' && <div>사용자 관리 컴포넌트</div>}
            {isAuthenticated && selectedMenu === 'category' && <div>카테고리 관리 컴포넌트</div>}
