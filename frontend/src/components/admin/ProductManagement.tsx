...
export default function ProductManagement() {
  ...
  const [categories, setCategories] = useState<{ id: number; name: string }[]>([ALL_CATEGORY_OPTION]);

  // 모달 상태 및 폼 상태
  const [modalOpen, setModalOpen] = useState(false);
  const [editingProduct, setEditingProduct] = useState<ProductWithCategoryName | null>(null);
  ...
  const openCreateModal = () => {
    ...
    setForm({ name: '', description: '', price: '', stock: '', category_id: 0, image_url: '' });
    setModalOpen(true);
    setFormError(null);
  };
  ...
  const columns = useMemo<ColumnDef<ProductWithCategoryName, any>[]>(() => [
    {
      ...
      header: '이미지',
      cell: ({ row }) => {
        const url = row.original.image_url && row.original.image_url.trim() !== '' ? row.original.image_url : '/coffee.jpeg';
        return <img src={url} alt={row.original.name} className="w-16 h-16 object-contain mx-auto" />;
      },
    },
    {
      accessorKey: 'name',
  ...
  return (
    <div className="w-full">
      ...
      {loading ? (
        ...
        <DataTable columns={columns} data={paged} rowClassName="border-b border-gray-200" />
      )}
      {/* 페이지네이션 */}
      {total > 0 && (
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
            <h2 className="text-lg font-bold mb-4">{editingProduct ? '상품 수정' : '상품 생성'}</h2>
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-3">
              {/* 상품명 */}
              <label className="font-semibold text-sm" htmlFor="modal-name">상품명</label>
              <input
                id="modal-name"
              ...
              />
              {/* 설명 */}
              <label className="font-semibold text-sm" htmlFor="modal-description">설명</label>
              <textarea
              ...
              />
              {/* 가격 */}
              <label className="font-semibold text-sm" htmlFor="modal-price">가격</label>
              <input
                id="modal-price"
                className="border rounded px-3 py-2"
                name="price"
              ...
              />
              {/* 재고 */}
              <label className="font-semibold text-sm" htmlFor="modal-stock">재고</label>
              <input
                id="modal-stock"
                className="border rounded px-3 py-2"
                name="stock"
              ...
              />
              {/* 카테고리 */}
              <label className="font-semibold text-sm" htmlFor="modal-category">카테고리</label>
              <select
                id="modal-category"
                className="border rounded px-3 py-2"
                name="category_id"
              ...
              >
                ...
                {categories.filter(c => c.id !== 0).map(opt => (
                  <option key={opt.id} value={opt.id}>{opt.name}</option>
                ))}
              </select>
              {/* 이미지 URL */}
              <label className="font-semibold text-sm" htmlFor="modal-image">이미지 URL</label>
              <input
                id="modal-image"
                className="border rounded px-3 py-2"
                name="image_url"
              ...
              <div className="flex gap-2 mt-2 justify-end">
                <Button type="submit" disabled={formLoading}>{editingProduct ? '수정' : '생성'}</Button>
                <Button type="button" variant="secondary" onClick={closeModal} disabled={formLoading}>취소</Button>
              </div>
            </form>
          </div>
        </div>
