...
import type { CategoryResponseDto, CategoryRequestDto } from '@/lib/backend/apiV1/api';
...
export default function CategoryManagement() {
  ...
  const [form, setForm] = useState<{ name: string; parentId?: number }>({ name: '', parentId: undefined });
  ...
  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm(f => ({ ...f, [name]: name === 'parentId' ? (value ? Number(value) : undefined) : value }));
  };
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setFormError(null);
    try {
      const payload: CategoryRequestDto = { name: form.name, parentId: form.parentId };
      if (editingCategory) {
  ...
  return (
    <div className="w-full">
      ...
      {loading ? (
        <div className="text-center py-16 text-gray-500">카테고리 목록을 불러오는 중...</div>
      ) : error ? (
        <div className="text-center py-16 text-red-500">{error}</div>
      ) : categories.length === 0 ? (
        <div className="text-center py-16 text-gray-500">등록된 카테고리가 없습니다.</div>
      ) : (
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
              <h2 className="text-lg font-bold mb-4">{editingCategory ? '카테고리 수정' : '카테고리 생성'}</h2>
              <form onSubmit={handleFormSubmit} className="flex flex-col gap-3">
                <label className="font-semibold text-sm" htmlFor="modal-name">이름</label>
                <input
                  id="modal-name"
                  ...
                  required
                />
                <label className="font-semibold text-sm" htmlFor="modal-parent">상위카테고리</label>
                <select
                  id="modal-parent"
                  className="border rounded px-3 py-2"
                  name="parentId"
                ...
                >
                  ...
                  {categories.filter(c => !editingCategory || c.id !== editingCategory.id).map(opt => (
                    <option key={opt.id} value={opt.id}>{opt.name}</option>
                  ))}
                </select>
                {formError && <div className="text-red-500 text-sm">{formError}</div>}
                <div className="flex gap-2 mt-2 justify-end">
                  <Button type="submit" disabled={formLoading}>{editingCategory ? '수정' : '생성'}</Button>
